...
from signal_lists import build_quality_tag
...
def evaluate_hundred_profile(alert: dict, analysis: dict | None = None) -> dict:
    ...
    if direction == "ALT":
        ...
        if (
            fair_edge is not None and total is not None and ppm is not None
            and -3 < fair_edge < 0 and 140 <= total < 170 and 4.2 <= ppm < 4.6
        ):
            rules.append("alt_edge_minus3_0_total_140_170_ppm_4_2_4_6")

    if direction == "ÜST":
        if (
            total is not None and ppm is not None and projected_gap is not None
            and total >= 170 and 3.8 <= ppm < 4.2 and -5 < projected_gap < 0
        ):
            rules.append("ust_total_170_ppm_3_8_4_2_gap_minus5_0")
        if fair_edge is not None and total is not None and total >= 170 and 10 <= diff < 12 and 0 < fair_edge < 3:
            rules.append("ust_total_170_diff_10_12_edge_0_3")
        if margin is not None and total is not None and projected_gap is not None and 4 <= margin <= 7 and total >= 170 and projected_gap <= -10:
...
def evaluate_telegram_profiles(alert: dict, analysis: dict | None = None) -> dict:
    """Evaluate and return the two profiles required for Telegram delivery."""
    analysis = analysis if isinstance(analysis, dict) else {}
    profile_alert = {**alert, **analysis}
    profile_alert["quality_label"] = build_quality_tag(profile_alert)["label"]

    hundred = evaluate_hundred_profile(profile_alert, analysis)
    profile_alert["hundred_profile"] = 1 if hundred.get("hundred_profile") else 0
    claude = evaluate_claude_ai(profile_alert, analysis)
    return {
        "hundred_profile": bool(hundred.get("hundred_profile")),
        "hundred_profile_rule": str(hundred.get("hundred_profile_rule") or ""),
        "claude_ai": str(claude.get("claude_ai") or ""),
        "claude_ai_rule": str(claude.get("claude_ai_rule") or ""),
    }
