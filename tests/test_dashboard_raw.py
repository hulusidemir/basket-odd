...
class DashboardRawTests(unittest.TestCase):
    ...
    def test_live_dto_adds_current_pace_projection(self):
        ...
        self.assertEqual(row["ppm_comparison"]["remaining_points"], 96)
    ...
    def test_reference_decision_display_is_frozen(self):
        ...
        self.assertEqual(row["barem_change"], 20)
        self.assertEqual(row["decision_change"], -20)
        frozen = self.dashboard._frozen_deleted_alert({
            **row, "diff": 999, "reference_total": 999, "display_snapshot": json.dumps(row),
        })
        self.assertEqual(frozen["decision_change"], -20)
        self.assertEqual(frozen["reference_total"], 200)
    ...
    def test_deleted_dto_leaves_missing_snapshot_fields_empty(self):
        ...
        self.assertIsNone(row["ppm_comparison"])
    ...
    def test_archive_pipeline_freezes_projection_for_deleted_page(self):
        ...
        self.assertEqual(snapshot["snapshot_meta"]["schema_version"], 2)
        self.assertEqual(snapshot["pace_projection"], 200.0)
        self.assertEqual(snapshot["ppm_comparison"]["required_ppm"], 3.84)
        self.assertEqual(snapshot["ppm_comparison"]["required_change_pct"], -23.2)
    ...
    def test_upcoming_fetch_button_stays_disabled_while_fetch_is_running(self):
        ...
        self.assertIn("if(fetchRequestPending||fetchRunning||clearPending)return", template)
    ...
    def test_dashboard_has_match_filter_signal_badge_and_row_action_states(self):
        ...
        self.assertIn("const arrivalTime = alert.signal_time", template)
    ...
    def test_dashboard_has_signal_statistics_modal_and_blacklist_controls(self):
        ...
        self.assertIn("function toggleSignalList(button)", template)
        self.assertIn("/team-history", template)
        template += (Path(self.dashboard.app.static_folder) / "signal_display.js").read_text(encoding="utf-8")
        self.assertIn("function historyTeamCard(team)", template)
        ...
        self.assertIn("Maç detayını aç", template)
        self.assertIn("modal-list-button", stylesheet)
        self.assertIn(".history-verdict.failed", stylesheet)
        self.assertIn('id="signalListForm"', template)
        self.assertIn('id="signalListsPanel"', template)
    ...
    def test_archive_shows_persisted_final_score_without_total_calculation(self):
        template = (Path(self.dashboard.app.template_folder) / "deleted_matches.html").read_text(encoding="utf-8")

        template += (Path(self.dashboard.app.static_folder) / "signal_display.js").read_text(encoding="utf-8")
        self.assertIn("function historyTeamCard(team)", template)
        self.assertNotIn("finalScoreTotal(a.final_score)", template)
        self.assertIn("esc(a.final_score||'–')", template)
