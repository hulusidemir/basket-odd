...
class UpcomingScraperAsyncTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retry_replaces_entire_bookmaker_tuple_and_ignores_listing_opening(self):
        scraper = UpcomingScraper()
        scraper._kickoff_in_allowed_window = lambda _value: True
        link = "https://m.aiscore.com/basketball/match-a-b/id-a"
        scraper._listing_rows_by_id["id-a"] = {
            "match_name": "A - B", "kickoff": "2026-09-09 18:30",
            "kickoff_source": "scheduled_mobile", "opening_total": 300,
        }
        scraper._read_odds_page = AsyncMock(side_effect=[
            {"match_name": "A - B", "opening": None, "prematch": 160, "bookmaker": "A"},
            {"opening": 170, "prematch": None, "bookmaker": "B"},
        ])
        row = await scraper._extract_one(_DetailContext(), link)
        self.assertEqual(scraper._read_odds_page.await_count, 2)
        self.assertEqual(row["opening_total"], 170)
        self.assertIsNone(row["prematch_total"])
        self.assertEqual(row["bookmaker"], "B")
