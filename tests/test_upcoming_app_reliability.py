...
class UpcomingAppReliabilityTests(unittest.TestCase):
            ...
            upcoming_app._fetch_state.update(
                {
                    ...
                    "report": None,
                    "error": None,
                    "timeout_seconds": upcoming_app.FETCH_TIMEOUT_SECONDS,
                }
            )
    ...
    def test_superseded_worker_never_writes_to_database(self):
        class SupersededScraper(_FakeScraper):
            async def fetch(self):
                with upcoming_app._fetch_lock:
                    upcoming_app._fetch_state.update(job_id=43, running=True)
                return await super().fetch()

        scraper = SupersededScraper(_complete_report(), [{"match_id": "old"}])
        with patch.object(upcoming_app, "Database", _FakeDatabase):
            upcoming_app._run_fetch_job(42, scraper, 60, "ignored.db")
        self.assertEqual(_FakeDatabase.instances, [])
        self.assertEqual(upcoming_app._fetch_state["job_id"], 43)
        self.assertTrue(upcoming_app._fetch_state["running"])

    def test_expired_worker_cannot_persist_even_without_status_poll(self):
        with upcoming_app._fetch_lock:
            upcoming_app._fetch_state["started_at"] = (
                datetime.now(timezone.utc) - timedelta(days=1)
            ).isoformat()
        with patch.object(upcoming_app, "Database", _FakeDatabase):
            upcoming_app._save_fetch_result(42, [{"match_id": "old"}], _complete_report(), "ignored.db")
        self.assertEqual(_FakeDatabase.instances, [])
        self.assertFalse(upcoming_app._fetch_state["running"])

    def test_list_poll_expires_stuck_fetch(self):
        with upcoming_app._fetch_lock:
            upcoming_app._fetch_state["started_at"] = (
                datetime.now(timezone.utc) - timedelta(days=1)
            ).isoformat()
        ...
        with patch.object(upcoming_app, "Database", _FakeDatabase):
            payload = app.test_client().get("/upcoming/api/list").get_json()
        self.assertFalse(payload["running"])
        self.assertEqual(payload["report"]["status"], "failed")
        self.assertTrue(payload["last_fetch_error"])

    def test_failed_report_is_exposed_as_error_even_without_exception(self):
        scraper = _FakeScraper({"status": "failed"}, [])
        with patch.object(upcoming_app, "Database", _FakeDatabase):
            upcoming_app._run_fetch_job(42, scraper, 60, "ignored.db")
        self.assertFalse(upcoming_app._fetch_state["running"])
        self.assertTrue(upcoming_app._fetch_state["error"])

    def test_thread_start_failure_does_not_leave_fetch_running(self):
        with upcoming_app._fetch_lock:
            upcoming_app._fetch_state["running"] = False
        app = Flask(__name__)
        app.register_blueprint(upcoming_app.upcoming_bp)
        with (
            patch.object(upcoming_app, "_build_scraper", return_value=_FakeScraper({}, [])),
            patch.object(upcoming_app.threading.Thread, "start", side_effect=RuntimeError("cannot start")),
        ):
            response = app.test_client().post("/upcoming/api/fetch")
